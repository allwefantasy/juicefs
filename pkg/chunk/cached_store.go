...
func (store *cachedStore) cleanupDelayStaging() {
	innerStores := store.bcache.(*cacheManager).stores
	type Pair struct {
		fs   map[string]string
		goal int64
	}

	fschan := make(chan Pair)

	computeFreeGoal := func(innerStore *cacheStore, br float32) int64 {
		total, _, _, _ := getDiskUsage(innerStore.dir)
		goal := innerStore.capacity * 80 / 100
		toFree := int64(float32(total) * (innerStore.freeRatio - br))
		if toFree > innerStore.used {
			goal = 0
		} else if innerStore.used-toFree < goal {
			goal = innerStore.used - toFree
		}
		return goal
	}

	for i := range innerStores {
		go func(i int) {
			innerStore := innerStores[i]
			br, fr := innerStore.curFreeRatio()
			if br < innerStore.freeRatio || fr < innerStore.freeRatio {
				fschan <- Pair{
					fs:   innerStore.scanStaging(),
					goal: computeFreeGoal(innerStore,br),
				}
			}
		}(i)
	}
	for range innerStores {
		pair := <-fschan
		var freed int64
		for key, path := range pair.fs {
